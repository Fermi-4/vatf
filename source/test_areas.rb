...
module TestAreas
  ...
  RtpInfo = Struct.new(:image, :path, :sources)
	...
	def get_rtps(rtp_val, source_dir = '/', result_dir = '/', platform = nil)
	  rtps = []
	  rtp_val.each do |level, image_hash|
      image_hash.each do |image, areas|
    		if areas['test_areas'].kind_of?(String)
    			rtps << RtpInfo.new(image, areas['test_areas'], areas['sources'])
    		else
          if areas['test_areas'][0].to_s.strip.downcase == 'all'
            ...
            base_dir.each do |current_area|
              ...
              get_matrices(current_dir,result_dir, level+'_'+current_area).each{|rtp| rtps << RtpInfo.new(image,rtp, areas['sources'])}
              get_matrices(current_dir+'/'+platform,result_dir, platform+'_'+level+'_'+current_area).each{|rtp| rtps << RtpInfo.new(image,rtp, areas['sources'])} if platform && File.exists?(current_dir.sub(/(\\|\/)$/,'')+'/'+platform)
            end
    			else
    				areas['test_areas'].each do |current_area|
    					...
    					get_matrices(current_dir,result_dir, level+'_'+current_area).each{|rtp| rtps << RtpInfo.new(image,rtp, areas['sources'])}
    					get_matrices(current_dir+'/'+platform,result_dir, platform+'_'+level+'_'+current_area).each{|rtp| rtps << RtpInfo.new(image,rtp, areas['sources'])} if platform && File.exists?(current_dir+'/'+platform)
    				end
    			end
        end
      end
