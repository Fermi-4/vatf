...
namespace NDde.Advanced
{
    ...
    {
        private static readonly TimeSpan MarshalTimeout = TimeSpan.FromSeconds(Double.Parse(Resources.DdeContextDefaultMarshalTimeoutSeconds));
        ...
        /// <summary>
        /// A bool indicating whether the context is initialized.
        /// </summary>
        ...
        /// <summary>
        /// A bool indicating whether the caller must use Invoke.
        /// </summary>
        ...
        {
            if (MarshalTimeout.TotalSeconds > 0) 
            {
                IAsyncResult ar = null;
                MarshalState state = new MarshalState();
                Delegate d = new MarshaledInvokeDelegate1(MarshaledInvoke);
                object[] a = { method, args, state };
                lock (state) 
                {
                    ar = Synchronizer.BeginInvoke(d, a);
                    if (!state.Executing.WaitOne(MarshalTimeout, false))
                    {
                        state.IsCanceled = true;
                        throw new DdeException(Resources.MarshalTimeoutMessage);
                    }
                }
                return Synchronizer.EndInvoke(ar);
            }
            else
            {
                Delegate d = new MarshaledInvokeDelegate1(MarshaledInvoke);
                object[] a = { method, args, null };
                return Synchronizer.Invoke(d, a);
            }
        }
        ...
        public IAsyncResult BeginInvoke(Delegate method, object[] args)
        {
            Delegate d = new MarshaledInvokeDelegate1(MarshaledInvoke);
            object[] a = { method, args, null };
            return Synchronizer.BeginInvoke(d, a);
        }
        ...
        private delegate object MarshaledInvokeDelegate1(Delegate method, object[] args, MarshalState state);

        private static object MarshaledInvoke(Delegate method, object[] args, MarshalState state)
        {
            if (state != null) 
            {
                state.Executing.Set();
        ...
        private sealed class MarshalState
        {
            private bool             _IsCanceled = false;
            private ManualResetEvent _Executing  = new ManualResetEvent(false);
            ...
            public ManualResetEvent Executing
            {
        ...
        {
            // TODO: Verify instance member thread-safety.
            ...
            private ThreadForm _Hidden   = null;
            
            public DdeThread()
            {
                _Hidden = new ThreadForm();
            ...
            private void Dispose(bool disposing)
            {
                ...
                if (disposing)
                {
                    if (Interlocked.CompareExchange(ref _ThreadId, 0, 0) == AppDomain.GetCurrentThreadId())
                    {
                        _Hidden.Dispose();
                    }
                    else
                    {
                        PostThreadMessage(Interlocked.CompareExchange(ref _ThreadId, 0, 0), WM_QUIT, IntPtr.Zero, IntPtr.Zero);
                        TimeSpan timeout = TimeSpan.FromSeconds(Double.Parse(Resources.DdeThreadTerminationWaitTimeoutSeconds));
            ...
            public bool InvokeRequired
            {
                get 
                {
                    if (Convert.ToBoolean(_Thread.ThreadState & ThreadState.Unstarted))
                    {
                        return true;
                    }
                    return _Hidden.InvokeRequired; 
                }
            }
            ...
            public object Invoke(Delegate method, object[] args)
            {
                ...
                return _Hidden.Invoke(method, args);
            }
            ...
            public IAsyncResult BeginInvoke(Delegate method, object[] args)
            {
                ...
                return _Hidden.BeginInvoke(method, args);
            }
            ...
            public object EndInvoke(IAsyncResult asyncResult)
            {
                ...
                return _Hidden.EndInvoke(asyncResult);
            }

            #pragma warning disable 0618
            private void Run()
            {
                Interlocked.Exchange(ref _ThreadId, AppDomain.GetCurrentThreadId());
            ...
            /// <threadsafety static="true" instance="false" />
            private sealed class ThreadForm : HiddenForm
            {
                private ManualResetEvent _Initialized = new ManualResetEvent(false);

                public ThreadForm() : base()
                {
                    this.Load += this.ThreadForm_Load;
                }
                ...
                private void ThreadForm_Load(object source, EventArgs e)
                {
                    _Initialized.Set();
                }
