...
namespace NDde.Foundation.Server
{
    ...
    internal abstract class DdemlServer : IDisposable
    {
        ...
        public DdemlServer(string service) : this(service, DdemlContext.GetDefault())
        {
        }
        ...
        internal bool ProcessCallback(DdemlTransaction transaction)
        {
            ...
            switch (t.uType)
            {
                ...
                case Ddeml.XTYP_EXECUTE:
                {
                    ...
                    string command = Encoding.ASCII.GetString(data, 0, data.Length - 1);
        ...
        public struct RequestResult
        {
            ...
            {
                _Result = data != null && data.Length > 0 ? Ddeml.DDE_FACK : Ddeml.DDE_FNOTPROCESSED;
                _Data = data;
            }
            ...
            public static IntPtr Register(int instanceId, string service)
            {
                lock (_Table) 
                {
                    ...
                    if (result != IntPtr.Zero)
                    {
                        ...
                        _Table.Add(serviceHandle, AppDomain.GetCurrentThreadId());
                    }
                    else
                    {
