...
namespace NDde.Foundation.Advanced
{
    ...
    internal sealed class DdemlContext : IDisposable
    {
        ...
        internal static DdemlContext GetDefault()
        {
            lock (_Instances)
            {
                DdemlContext context = _Instances[AppDomain.GetCurrentThreadId()];
                if (context == null)
                {
                    context = new DdemlContext();
                    _Instances.Add(AppDomain.GetCurrentThreadId(), context);
                }
                return context;
            }
        }
        ...
        {
            // Make sure the current thread has a windows message loop.
            if (!Application.MessageLoop)
            {
                throw new InvalidOperationException(Resources.NoMessageLoopMessage);
            }
            ...
            public static int Initialize(Ddeml.DdeCallback pfnCallback, int afCmd)
            {
                lock (_Table) 
                {
                    ...
                    if (instanceId != 0)
                    {
                        ...
                        _Table.Add(instanceId, AppDomain.GetCurrentThreadId());
                    }
