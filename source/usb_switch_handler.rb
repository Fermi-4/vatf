...
class UsbSwitchHandler
	...
	def disconnect(switch_name)
		@usb_switch_controller[switch_name.to_s.downcase].disconnect()
	end

	def select_input(input)
    input.each {|switch_name,port|
      @usb_switch_controller[switch_name.to_s.downcase].select_input(port)
    }
	end
