...
module DemoHandlers
    class DemoLinuxClientDM365 < DemoHandlers::DemoDefaultClient
      ...
      exec_func(params.merge({"function" => "decode"}))
        ...
        def exec_func(params)
            module_name = params["function"] == 'encode_decode'? 'loadmodules_sd.sh' : 'loadmodules_hd.sh'
      @target.send_cmd("./#{module_name}", @target.prompt)
        ...
        if params["video_file"] && params["function"] == 'decode'
          ...
          if @target.is_timeout
            @target.send_cmd("\cZ",@target.prompt,5)
            @target.send_cmd("pkill -9 #{command.sub(/^\.\//,'').split(/ +/)[0]}",@target.prompt,5)
