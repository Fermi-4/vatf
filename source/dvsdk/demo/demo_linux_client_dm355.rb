...
module DemoHandlers
    class DemoLinuxClientDM355 < DemoHandlers::DemoDefaultClient
        ...
        def initialize(platform_info, log_path = nil)
            ...
            @decode_params = {
                ...
                #'frame_skip'			=> {'name' => '', 'values' => {'yes' => '-f', 'no' => ''}},
                'enable_keyboard'		=> {'name' => '', 'values' => {'yes' => '-k', 'no' => ''}},
                'enable_remote'			=> {'name' => '', 'values' => {'yes' => '-i', 'no' => ''}},
            }
        ...
        def exec_func(params)
            ...
            @target.send_cmd("rmmod dm350mmap", @target.prompt)
            ...
            t = Thread.new {
                begin
            	    ...
            	    #10.times {puts "Waiting for file write to complete"; sleep 1}
            	    if params["video_file"] && (params["function"] == 'encode' || params["function"] == 'multi_encode')
                        puts "\nCopying video file: #{@target.samba_root_path}#{@target.executable_path.gsub(/\//,'\\')}\\#{File.basename(params['video_file'])} to #{params["video_file"]}"
                    ...
                    if params["speech_file"] && (params["function"] == 'encode' || params["function"] == 'multi_encode')
                        puts "\nCopying speech file: #{@target.samba_root_path}#{@target.executable_path.gsub(/\//,'\\')}\\#{File.basename(params['speech_file'])} to #{params["speech_file"].gsub(/\.g711/,'.pcm')}"
