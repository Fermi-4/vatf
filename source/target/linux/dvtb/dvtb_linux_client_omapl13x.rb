    ...
    class DvtbLinuxClientOmapL13x < DvtbHandlers::DvtbLinuxClient
        ...
        def translate_value(params)
          case params['Class']
            ...
            when 'engine': 
              case params['Param']
                when 'name' : params['Value'] == 'encdec'? 'encodedecode' :  'encodedecode'
                else params['Value']
              end     
            when /vid[end]+c/   
              case params['Param']
            ...
            when /aud[end]+c/
              case params['Param']
                ...
                when /endianness/i : get_xdm_data_format(params['Value'].strip.downcase)
                when /ChannelMode/i : get_audio_channel_mode(params['Value'].to_s)
                else params['Value']
              end  
            when /mpeg4ext[end]+c/ 
              case params['Param']
            ...
            when /jpeg[endxt]+c/   
              case params['Param']
                ...
                when 'captureWidth'  : '0' #get_adjusted_width(get_video_format_width(params['Value'].to_s)).to_s
                when /width/i : get_adjusted_width(params['Value'].strip).to_s
                else params['Value']
              end  
            else params['Value']
        ...
        def get_codec_type(codec)
          case codec.strip.downcase
            when /h264enc/i : 'h264enc'
            when /h264dec/i : 'h264dec'
            when /mpeg4enc/i : 'mpeg4enc'
            when /mpeg4dec/i : 'mpeg4dec'
            when /jpegenc/i : 'jpegenc'
            when /jpegdec/i : 'jpegdec'
            else codec 
          end
        end
